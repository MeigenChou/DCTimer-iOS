...
+(void) initCent1 {
    ...
    [c set:0];
    for (int i=0; i<48; i++) {
        finish[syminv4[i]] = [c get];
        [c rot:0];
        if (i%2==1) [c rot:1];
        if (i%8==7) [c rot:2];
        if (i%16==15) [c rot:3];
    }
    raw2sym = (int *)malloc(735471 * sizeof(int));
    //int occ[22984];
    NSLog(@"init sym2raw...");
    int count = 0;
    for (int i=0; i<735471; i++) {
        if ((raw2sym[i] & 1) == 0) {
        //if ((occ[(unsigned)i>>5]&(1<<(i&0x1f))) == 0) {
            [c set:i];
            for (int j=0; j<48; j++) {
                int idx = [c get];
                //occ[(unsigned)idx>>5] |= (1<<(idx&0x1f));
                raw2sym[idx] = (count << 7) | (syminv4[j] << 1) | 1;
                [c rot:0];
                if (j%2==1) [c rot:1];
                if (j%8==7) [c rot:2];
                if (j%16==15) [c rot:3];
