...
- (void) layoutSubviews
{
	...
	CGFloat indicatorLoc = kContentInsetX + (self.value * (self.bounds.size.height - 2 * kContentInsetX));
	indicator.center = CGPointMake(CGRectGetMidX(self.bounds), indicatorLoc);
}
...
{
	float percent = ([touch locationInView: self].y - kContentInsetX)
